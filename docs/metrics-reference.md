...
| **Description** | Same as `mtv_migrations_status_total` but adds a `plan` label (plan UID) for per-plan correlation. |
...
The `phase` label contains `Completed` for succeeded plans, `Executing` for
in-progress plans, or a comma-separated list of error phases for failed plans.
